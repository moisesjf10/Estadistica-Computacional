...
Observamos que en este caso el número óptimo de vecinos de $k=$ `r mod_knn$bestTune$k`
...
### Análisis de los resultados
En conjunto, los resultados muestran que KNN funciona bien para el problema de clasificación del conjunto de datos `iris`. Las matrices de confusión muestran una exactitud alta tanto en entrenamiento, 96.67%, como en test, 93.33%. Además, al ser ambos valores similares, no parece haber un sobreajuste importante, por lo que el modelo obtiene una frontera de decisión suficientemente flexible, pero sin depender en exceso de puntos individuales.
...
Gradient Boosting es un método que combina secuencialmente varios árboles de decisión poco profundos. Cada nuevo árbol se entrena para corregir los errores cometidos por los árboles anteriores, mejorando progresivamente el modelo.

A diferencia de Random Forest, donde los árboles se construyen de forma independiente, Gradient Boosting los genera de manera iterativa. Matemáticamente, el modelo final puede expresarse como:

$$
F_M(x)=\sum_{m=1}^{M}\gamma_m h_m(x),
$$

donde $h_m(x)$ es el árbol construido en la iteración $m$, $\gamma_m$ su peso y $M$ el número total de árboles. Cada árbol se incorpora siguiendo la dirección del gradiente negativo de una función de pérdida.

Los principales hiperparámetros que se optimizarán son: **`n.trees`**, que controla el número de árboles del modelo; **`interaction.depth`**, que determina la profundidad máxima de cada árbol; y **`shrinkage`**, que regula la contribución de cada árbol mediante la tasa de aprendizaje.

Gradient Boosting destaca por su capacidad para capturar relaciones no lineales y por no requerir supuestos de linealidad ni escalado previo de las variables, lo que lo hace adecuado para problemas de clasificación como `iris`.
...
Las matrices de confusión muestran que Gradient Boosting obtiene un rendimiento muy elevado sobre el conjunto de entrenamiento. El modelo clasifica correctamente 115 de las 120 observaciones, alcanzando una exactitud aproximada del 95,83%. La especie `setosa` se identifica perfectamente en todos los casos, lo que refleja la clara separación existente entre esta clase y las demás. Los únicos errores aparecen entre las especies `versicolor` y `virginica`, que presentan características morfológicas más similares y, por tanto, una mayor zona de solapamiento.
...
La especie setosa se identifica correctamente en todos los casos. Los errores de clasificación se concentran entre las especies versicolor y virginica, que presentan características morfológicas más similares. 

La diferencia de rendimiento entre entrenamiento y test es reducida, lo que sugiere que el modelo presenta una buena capacidad de generalización y no muestra indicios claros de sobreajuste. Esto indica que la estrategia de validación cruzada utilizada para seleccionar los hiperparámetros obtiene un equilibrio adecuado entre complejidad y capacidad predictiva. La frontera de decisión confirma esta interpretación, ya que se observa una separación clara de la especie setosa, mientras que la región que separa versicolor y virginica presenta una estructura más compleja.

En conjunto, los resultados muestran que Gradient Boosting constituye una alternativa potente y flexible para este problema de clasificación.
