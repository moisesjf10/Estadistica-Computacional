...
Definimsos la siguiente función para dibujar matrices de confusión en los distintos métodos.
```{r}
...
El modelo se ajusta mediante la función `train()` de `caret`, usando el método `"multinom"` y probando distitnos valores del parámetro de regularización `decay`.
...
La regresión logística multinomial obtiene un rendimiento alto tanto en entrenamiento como en test. La especie `setosa` se clasifica correctamente en todos los casos, mientras que los errores aparecen únicamente entre `versicolor` y `virginica`, que son las dos clases con mayor solapamiento en las variables del pétalo.
