...
header-includes:
  ...
  - \usepackage{mathpazo}
  - \usepackage{titlesec}
  - \renewcommand{\maketitle}{} # Evita que el PDF duplique el título
  - \newcommand{\sectionbreak}{\clearpage}
  - \newcommand{\subsectionbreak}{\clearpage}
output:
  html_document:
    toc: true
    toc_float: true # Crea un menú lateral flotante para el índice en HTML
    toc_depth: 3
    theme: readable
    highlight: tango
  pdf_document:
...
A la hora de crear un árbol de decisión, lo normal es coger la variable que permite diferenciar mejor las instancias de las clases. Sin embargo, si utilizamos esta perspectiva para crear los diferentes árboles del bosque, se heredarían los errores obtenidos utilizando esta forma de división. Por esta razón, random forest incluye un hiperparámetro: el número de características a tener en cuenta en cada árbol, las cuales se eligirán de forma aleatoria a la hora de construir los árboles. A este parámetro lo llamaremos ``mtry`` y, como el dataset tiene 4 características, $mtry \in \{1,2,3,4\}$.


Una vez creado un bosque con $S$ árboles, dada una nueva instancia $x$, el método ejecuta la entrada en cada uno de los árboles $\hat{f_i}(x)$ y devuelve el valor más frecuente. Es decir:
...
#Visualizacióndelafronteradedecisión
ggplot() +
...
plot_cm(cm_gbm_train, "Gradient Boosting", "Entrenamiento")
...
plot_cm(cm_gbm_test, "Gradient Boosting", "Test")
```
...
plot_cm(cm_nnet, "Red Neuronal", "Test")
